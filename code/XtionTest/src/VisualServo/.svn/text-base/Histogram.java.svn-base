...
public class Histogram {
	...
	public static float[][] makeHistogram(Image source, boolean hsbHistogram) {
		...
		// Begin Student Code
		if (!hsbHistogram){
			int red_bin, green_bin, blue_bin, red, green, blue;
			
			for (int i = 0; i < width; i++){
				for (int j = 0; j < height; j++){
					red = Image.pixelByteToInt(source.getPixelRed(i, j));
					green = Image.pixelByteToInt(source.getPixelGreen(i, j));
					blue = Image.pixelByteToInt(source.getPixelBlue(i, j));
					
					red_bin = (int) (red / scale);
					green_bin = (int) (green / scale);
					blue_bin = (int) (blue / scale);

					histogram[red_bin][0]++;
					histogram[green_bin][1]++;
					histogram[blue_bin][2]++;
				}
			}	
		} else {
			int hue_bin, sat_bin, bright_bin, pix;
			int red, green, blue;
			float[] hsb;
			...
			for (int i = 0; i < width; i++){
				for (int j = 0; j < height; j++){
					...
					// Subtracted 0.001 to deal with the case that h, s or b is exactly 1.0 (without it it causes an index out of array issue)
					hue_bin = (int) (hsb[0] * Math.min(width - 0.001, 256 - 0.001));
					sat_bin = (int) (hsb[1] * Math.min(width - 0.001, 256 - 0.001));
					bright_bin = (int) (hsb[2] * Math.min(width - 0.001, 256 - 0.001));

					histogram[hue_bin][0]++;
					histogram[sat_bin][1]++;
					histogram[bright_bin][2]++;
				}
			}
		}
